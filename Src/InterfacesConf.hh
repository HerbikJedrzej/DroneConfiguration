...
SPI spi3(
	gpio, 
	[](uint8_t *pTxData, uint8_t *pRxData, uint16_t Size)->BusStatus{
		auto returned = HAL_SPI_TransmitReceive_DMA(&hspi3, pTxData, pRxData, Size);
		if(returned == HAL_StatusTypeDef::HAL_OK)
			return BusStatus::OK;
		if(returned == HAL_StatusTypeDef::HAL_BUSY)
			return BusStatus::BUSY;
	...
	[](uint8_t *pTxData, uint8_t *pRxData, uint16_t Size, uint32_t Timeout)->BusStatus{
		auto returned = HAL_SPI_TransmitReceive(&hspi3, pTxData, pRxData, Size, Timeout);
		if(returned == HAL_StatusTypeDef::HAL_OK)
			return BusStatus::OK;
		if(returned == HAL_StatusTypeDef::HAL_BUSY)
			return BusStatus::BUSY;
	...
	[]()->bool{return(HAL_SPI_GetState(&hspi3) != HAL_SPI_StateTypeDef::HAL_SPI_STATE_READY);},
	[]()->bool{return(HAL_SPI_Init(&hspi3) == HAL_OK);},
	HAL_Delay
);
...
I2C i2c2(
	[](uint8_t DevAddress, uint16_t MemAddress, uint16_t MemAddSize, uint8_t *pData, uint16_t Size)->BusStatus{
		auto returned = HAL_I2C_Mem_Write_DMA(&hi2c2, DevAddress, MemAddress, MemAddSize, pData, Size);
		if(returned == HAL_StatusTypeDef::HAL_OK)
			return BusStatus::OK;
		if(returned == HAL_StatusTypeDef::HAL_BUSY)
			return BusStatus::BUSY;
	...
	[](uint8_t DevAddress, uint16_t MemAddress, uint16_t MemAddSize, uint8_t *pData, uint16_t Size)->BusStatus{
		auto returned = HAL_I2C_Mem_Read_DMA(&hi2c2, DevAddress, MemAddress, MemAddSize, pData, Size);
		if(returned == HAL_StatusTypeDef::HAL_OK)
			return BusStatus::OK;
		if(returned == HAL_StatusTypeDef::HAL_BUSY)
			return BusStatus::BUSY;
	...
	[](uint8_t DevAddress, uint16_t MemAddress, uint16_t MemAddSize, uint8_t *pData, uint16_t Size, uint32_t Timeout)->BusStatus{
		auto returned = HAL_I2C_Mem_Write(&hi2c2, DevAddress, MemAddress, MemAddSize, pData, Size, Timeout);
		if(returned == HAL_StatusTypeDef::HAL_OK)
			return BusStatus::OK;
		if(returned == HAL_StatusTypeDef::HAL_BUSY)
			return BusStatus::BUSY;
	...
	[](uint8_t DevAddress, uint16_t MemAddress, uint16_t MemAddSize, uint8_t *pData, uint16_t Size, uint32_t Timeout)->BusStatus{
		auto returned = HAL_I2C_Mem_Read(&hi2c2, DevAddress, MemAddress, MemAddSize, pData, Size, Timeout);
		if(returned == HAL_StatusTypeDef::HAL_OK)
			return BusStatus::OK;
		if(returned == HAL_StatusTypeDef::HAL_BUSY)
			return BusStatus::BUSY;
	...
	[]()->bool{return(HAL_I2C_GetState(&hi2c2) != HAL_I2C_StateTypeDef::HAL_I2C_STATE_READY);},
	HAL_Delay
);

I2C i2c3(
	[](uint8_t DevAddress, uint16_t MemAddress, uint16_t MemAddSize, uint8_t *pData, uint16_t Size)->BusStatus{
		auto returned = HAL_I2C_Mem_Write_DMA(&hi2c3, DevAddress, MemAddress, MemAddSize, pData, Size);
		if(returned == HAL_StatusTypeDef::HAL_OK)
			return BusStatus::OK;
		if(returned == HAL_StatusTypeDef::HAL_BUSY)
			return BusStatus::BUSY;
	...
	[](uint8_t DevAddress, uint16_t MemAddress, uint16_t MemAddSize, uint8_t *pData, uint16_t Size)->BusStatus{
		auto returned = HAL_I2C_Mem_Read_DMA(&hi2c3, DevAddress, MemAddress, MemAddSize, pData, Size);
		if(returned == HAL_StatusTypeDef::HAL_OK)
			return BusStatus::OK;
		if(returned == HAL_StatusTypeDef::HAL_BUSY)
			return BusStatus::BUSY;
	...
	[](uint8_t DevAddress, uint16_t MemAddress, uint16_t MemAddSize, uint8_t *pData, uint16_t Size, uint32_t Timeout)->BusStatus{
		auto returned = HAL_I2C_Mem_Write(&hi2c3, DevAddress, MemAddress, MemAddSize, pData, Size, Timeout);
		if(returned == HAL_StatusTypeDef::HAL_OK)
			return BusStatus::OK;
		if(returned == HAL_StatusTypeDef::HAL_BUSY)
			return BusStatus::BUSY;
	...
	[](uint8_t DevAddress, uint16_t MemAddress, uint16_t MemAddSize, uint8_t *pData, uint16_t Size, uint32_t Timeout)->BusStatus{
		auto returned = HAL_I2C_Mem_Read(&hi2c3, DevAddress, MemAddress, MemAddSize, pData, Size, Timeout);
		if(returned == HAL_StatusTypeDef::HAL_OK)
			return BusStatus::OK;
		if(returned == HAL_StatusTypeDef::HAL_BUSY)
			return BusStatus::BUSY;
	...
	[]()->bool{return(HAL_I2C_GetState(&hi2c3) != HAL_I2C_StateTypeDef::HAL_I2C_STATE_READY);},
	HAL_Delay
);
